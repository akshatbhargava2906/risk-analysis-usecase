...
1. User selects a sample PDF or uploads one
2. PDF bytes → base64 → Claude Messages API (type: document)
3. Claude returns strict JSON: `{patient_name, report_date, indicators[]}`
4. Each indicator has: `name`, `value`, `unit`, `reference_range`, `status` (normal|abnormal|critical), `note`
5. Risk score computed (Stage 2)
6. Second Claude call: score + flagged indicators → SUMMARY / FLAGGED INDICATORS / RECOMMENDATIONS
